...
CFCalendar366 <- R6::R6Class("CFCalendar366",
  ...
  public = list(
    ...
    date2offset = function(x) {
      yd0 <- c(0L, 31L, 60L, 91L, 121L, 152L, 182L, 213L, 244L, 274L, 305L, 335L) # days diff of 1st of month to 1 January
      (x$year - self$origin$year) * 366L + yd0[x$month] - yd0[self$origin$month] + x$day - self$origin$day
    },
    ...
    offset2date = function(x) {
      month <- c(31L, 29L, 31L, 30L, 31L, 30L, 31L, 31L, 30L, 31L, 30L, 31L)

      # First process full years over the vector
      yr <- self$origin$year + (x %/% 366L)
      x <- x %% 366L
